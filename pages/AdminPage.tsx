...
const AdminPage: React.FC<AdminPageProps> = ({ profile }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <div>
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={handleAddTestMoney} className="bg-yellow-500/10 text-yellow-500 border border-yellow-500/20 px-4 py-2 rounded-xl text-[10px] font-black hover:bg-yellow-500 hover:text-black transition-all">KÍCH HOẠT TIỀN TỶ</button>
          </div>
        </div>
      ...
      {loading ? (
        <div className="text-center py-20 text-gray-500 font-bold uppercase text-[10px] tracking-widest animate-pulse">Đang tải dữ liệu hệ thống...</div>
      ) : (
        <>
              ...
              <div className="lg:col-span-1">
                <div className="bg-[#151a24] p-8 rounded-[32px] border border-gray-800 sticky top-24">
                  ...
                  <form onSubmit={handleSubmitTask} className="space-y-4">
                    ...
                    {editingTaskId && <button type="button" onClick={() => setEditingTaskId(null)} className="w-full text-gray-500 font-bold text-xs mt-2">Hủy</button>}
                  </form>
                </div>
              </div>
              <div className="lg:col-span-2 space-y-4">
                 {tasks.map(t => (
                   <div key={t.id} className="bg-[#151a24] p-6 rounded-3xl border border-gray-800 flex justify-between items-center group">
                    ...
                    {activeTab === 'members' ? members.map(m => (
                      ...
                      <tr key={w.id} className="border-b border-gray-800/50">
                        <td className="py-6">
                          ...
                          <p className="text-blue-500 font-black text-sm">{Number(w.amount).toLocaleString()}đ</p>
                        </td>
                        <td className="py-6">

...
const TasksPage: React.FC<TasksPageProps> = ({ profile, refreshProfile }) => {
  ...
  // 2. LOGIC RESET Ô NHẬP MÃ KHI HỦY
  ...
  const checkDailyLimit = async (taskId: string, maxPerDay: number) => {
    ...
    if (error) {
      ...
      return false; // Cho phép làm nếu lỗi DB để tránh chặn oan
    }
  ...
  const startTask = async (provider: typeof ADMIN_CONFIG.PROVIDERS[0]) => {
    ...
    try {
      // KIỂM TRA GIỚI HẠN LƯỢT LÀM
      const canProceed = await checkDailyLimit(provider.id, provider.max_per_day);
      if (!canProceed) {
      ...
      // Lưu user_id cố định theo yêu cầu
      ...
      // Fallback: Chuyển hướng sang link dự phòng
  ...
  const handleVerify = async (providerId: string) => {
    ...
    try {
      // 4. BẢO MẬT & CHỐNG GIAN LẬN QUA RPC (Transaction)
      ...
      if (data.success) {
        ...
        // RESET Ô NHẬP MÃ KHI THÀNH CÔNG
        setVerificationCode('');
        setVerifyingTaskId(null);
        ...
        // Giữ nguyên ô nhập mã để người dùng sửa nếu thất bại
      }
    } catch (err: any) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="mb-12">
        ...
        <p className="text-gray-500 mt-4 max-w-2xl font-medium">Làm nhiệm vụ mỗi ngày để nhận thưởng. Giới hạn lượt làm reset sau 00:00 hàng ngày.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {ADMIN_CONFIG.PROVIDERS.map((provider) => (
          <div key={provider.id} className="bg-[#151a24] rounded-[40px] p-8 border border-gray-800 hover:border-blue-500/50 transition-all shadow-xl group relative overflow-hidden flex flex-col justify-between min-h-[350px]">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mb-6">
                ...
                <span className="bg-blue-500/10 text-blue-500 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-wider">Tối đa: {provider.max_per_day}/ngày</span>
              </div>
            </div>
      ...
      <div className="mt-24 p-12 bg-blue-600/5 border border-blue-500/10 rounded-[48px] relative overflow-hidden group">
        <h4 className="text-white text-xl font-bold mb-8">Lưu ý quan trọng:</h4>
        <div className="grid md:grid-cols-2 gap-10">
           <ul className="space-y-6">
              <li className="flex gap-6 items-start">
                 <span className="text-blue-500 font-black">1.</span>
                 <p className="text-gray-500 text-sm">Vượt link rút gọn để đến trang đích <b className="text-blue-400">avudev-verifi.blogspot.com</b>.</p>
              </li>
              <li className="flex gap-6 items-start">
                 <span className="text-blue-500 font-black">2.</span>
                 <p className="text-gray-500 text-sm">Lấy mã xác nhận <b className="text-white">6 số</b> nằm ở cuối mỗi bài viết.</p>
              </li>
           </ul>
           <ul className="space-y-6">
              <li className="flex gap-6 items-start">
              ...
              <li className="flex gap-6 items-start">
                 ...
                 <p className="text-gray-500 text-sm">Nghiêm cấm hành vi sử dụng Tool/Cheat, hệ thống sẽ tự động <b className="text-red-500">Khóa tài khoản</b>.</p>
              </li>
           </ul>
        </div>
      </div>
